...
ifile_2d_name = 'hill_2D_optimized_5'
version = '0'
...
def visualize_open3d(points_2d, points_3d):
    points = []
    lines = []
    colors = []
    for i, p in enumerate(points_2d):
        points.append([p['x'], p['y'], 0])

    for i, p in enumerate(points_2d[:-1]):
        lines.append([i, i+1])

    for pp in points_3d:
        i = len(points)
        points.append([pp[0][0], pp[0][1], 0])
        points.append([pp[1][0], pp[1][1], 0])
        lines.append([i, i+1])
...
if visualization:
    visualize_open3d(points_2d, points_gap)
